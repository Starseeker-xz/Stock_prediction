...
class ModelConfig:
    ...
    # 融合前分支归一化
    # 注意：预测 Close 时 short 分支幅值本身携带价格水平信息，
    # 若对 short 也做 LayerNorm，容易退化成“输出常数”。
    BRANCH_LAYER_NORM = True
    # 'all'：short/medium/long 全归一化
    # 'aux_only'：仅 medium/long 归一化（推荐）
    # 'none'：不做归一化
    BRANCH_LAYER_NORM_MODE = 'aux_only'

    # gated_short_mlp 参数
    GATE_HIDDEN_SIZE = 32
    GATE_INIT_BIAS = -1.0  # 越小越倾向于初始不吸收（sigmoid 后接近 0）
    GATE_DETACH_AUX = False
    
    HIDDEN_SIZE_LONG = 64    # BiLSTM hidden size
    HIDDEN_SIZE_MEDIUM = 64 # LSTM hidden size
    TCN_CHANNELS = [32, 64, 64, 64] # TCN layers
