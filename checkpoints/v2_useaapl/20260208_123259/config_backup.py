...
class ModelConfig:
    ...
    BASE_FEATURE_COLS = ['Open', 'High', 'Low', 'Close', 'Volume', 'Open_logret', 'High_logret', 'Low_logret', 'Close_logret']
    ...
    # 是否归一化
    # 时间特征默认不归一化（已在 [-1, 1]）
    FEATURE_NORMALIZE_MASK_LONG = [True] * len(BASE_FEATURE_COLS) + [False] * len(TIME_FEATURES_LONG)
    FEATURE_NORMALIZE_MASK_SHORT_MEDIUM = [True] * len(BASE_FEATURE_COLS) + [False] * len(TIME_FEATURES_SHORT_MEDIUM)
